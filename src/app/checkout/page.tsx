...
export default function CheckoutPage() {
  ...
  if (checkoutStatus === "success") {
    return (
      <>
        <TopNavBar />
        <main className="pt-40 pb-20 px-4 min-h-screen flex flex-col items-center justify-center text-center bg-white">
          <div className="w-24 h-24 bg-primary/5 rounded-full flex items-center justify-center mb-10 text-primary animate-pulse">
            <CheckCircle2 size={48} strokeWidth={1} />
          </div>
          <p className="font-label-caps text-[10px] tracking-[0.3em] text-secondary mb-4 uppercase">Transaction Complete</p>
          <h1 className="font-headline-lg text-5xl md:text-7xl mb-8 tracking-tighter">ORDER RECEIVED</h1>
        ...
        <div className="flex flex-col lg:flex-row gap-20">
          ...
          <div className="flex-1 max-w-2xl">
            <header className="mb-16">
              ...
              <h1 className="font-headline-lg text-5xl uppercase tracking-tighter">CHECKOUT</h1>
            </header>
            ...
            <form onSubmit={handleSubmit} className="space-y-12">
              {step === 1 ? (
                ...
                <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  <div className="bg-surface p-12 border border-primary/5 space-y-6">
                    <h2 className="font-label-caps text-[11px] uppercase tracking-widest mb-8">Review Selection</h2>
                    <div className="space-y-4">
                      <p className="font-body-md flex justify-between"><span>Ship to:</span> <span className="text-on-surface-variant">{formData.customerName}</span></p>
                      <p className="font-body-md flex justify-between"><span>Address:</span> <span className="text-on-surface-variant">{formData.address}, {formData.city}</span></p>
                      <p className="font-body-md flex justify-between"><span>Contact:</span> <span className="text-on-surface-variant">{formData.email}</span></p>
                    </div>
              ...
              <div className="flex flex-col gap-6 pt-10">
                <button 
                  ...
                  className="w-full bg-primary text-on-primary py-6 font-button text-button uppercase tracking-[0.2em] hover:bg-black transition-all flex items-center justify-center gap-4 relative overflow-hidden group"
                >
          ...
          <div className="lg:w-96">
            <div className="bg-surface p-10 border border-primary/5 sticky top-32">
              <h2 className="font-label-caps text-[11px] mb-8 border-b border-primary/10 pb-4 uppercase tracking-widest">Bag Summary</h2>
              <div className="space-y-8 mb-12 max-h-[40vh] overflow-y-auto pr-4 hide-scrollbar">
                {items.map((item) => (
                  <div key={item.product._id} className="flex gap-4">
                    <div className="w-16 h-20 relative bg-surface-container shrink-0">
                      <Image src={item.product.imageUrl} alt={item.product.name} fill className="object-cover" />
                    </div>
                    <div className="flex-1 flex flex-col justify-center">
                      <h3 className="font-label-caps text-[10px] uppercase mb-1">{item.product.name}</h3>
                      <p className="text-[12px] text-on-surface-variant">Qty: {item.quantity}</p>
                      <p className="text-[12px] font-medium mt-1">${(item.product.price * item.quantity).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="space-y-4 border-t border-primary/10 pt-8">
                ...
                <div className="flex justify-between font-headline-md text-2xl pt-4 border-t border-primary/5">
                  <span>Total</span>
                  <span>${totalPrice().toLocaleString()}</span>
                </div>
              </div>
